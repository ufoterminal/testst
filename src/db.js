...
const local=embedded?new PGlite(process.env.DATA_DIR||'./data'):null;
export const storage=embedded?'pglite':'postgres';
export const pool=embedded?{query:async(sql,params=[])=>{await local.waitReady;return params.length?local.query(sql,params):local.exec(sql).then(x=>x.at(-1)||{rows:[]})},end:()=>local.close()}:new pg.Pool({connectionString:url,max:10});
